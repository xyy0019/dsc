...
static int colorimetry;
static int colorDifferenceSiting;
...
static  cmdarg_t cmd_args[] = {
	...
	{ PARG,  &hdrDpxFileOutput,   "HDR_DPX_FILE_OUTPUT",     "-hdpx",  0,  0 },    // output HDR DPX (ST 268-2) files
	{ PARG,  &transferFunction,   "TRANSFER_FUNCTION",  "-tf", 0, 0 },  // Transfer function for HDR DPX output
	{ PARG,  &colorimetry,         "COLORIMETRY",     "-clmtry",  0,  0 },    // Colorimetry for HDR DPX output
	{ PARG,  &colorDifferenceSiting, "COLOR_DIFF_SITING",   "-cds", 0, 0 },      // Color difference siting for HDR DPX output
	{ PARG,  &dscFileOutput,      "DSC_FILE_OUTPUT",     "-dfo", 0,  0 },  // Output DSC files if FUNCTION = 0
	...
	{ PARG,  &bitDepthConvRounding, "BIT_DEPTH_CONV_ROUNDING", "-bdcr", 0,  0 },  // 0 = truncate when changing bit depths, 1 = round when changing bit depths
...
void split_base_and_ext(char *infname, char *base_name, char **extension)
{
	...
	for (zz = (int)strlen(infname)-1; zz >= 0; --zz)
	{
...
int compute_rc_parameters(dsc_cfg_t *dsc_cfg, int pixelsPerGroup, int numSsps)
{
	...
	if (dsc_cfg->convert_rgb)
		num_extra_mux_bits = (numSsps*(dsc_cfg->mux_word_size + (4*bitsPerComponent+4)-2));
	else if (!dsc_cfg->native_422) // YCbCr
		num_extra_mux_bits = (numSsps*dsc_cfg->mux_word_size + (4*bitsPerComponent+4) + 2*(4*bitsPerComponent) - 2);
	else
		num_extra_mux_bits = (numSsps*dsc_cfg->mux_word_size + (4*bitsPerComponent+4) + 3*(4*bitsPerComponent) - 2);
	sliceBits = 8 * dsc_cfg->chunk_size * dsc_cfg->slice_height;
	while ((num_extra_mux_bits>0) && ((sliceBits - num_extra_mux_bits) % dsc_cfg->mux_word_size))
		num_extra_mux_bits--;
...
void populate_pps(dsc_cfg_t *dsc_codec, int *slicew, int *sliceh)
{
	...
	if (dsc_codec->bits_per_component <= 10)
		dsc_codec->mux_word_size = 48;
	else
		dsc_codec->mux_word_size = 64;  // 16bpc needs some minor syntax modifications to ensure this works
...
#endif
{
	...
	char frmnumstr[13];
	int type;
	HDRDPXFILEFORMAT hdrdpx_f;
	HDRDPXSBMDATA    hdrdpx_sbm;
	HDRDPXUSERDATA   hdrdpx_user;

	printf("Display Stream Compression (DSC) reference model version 1.63\n");
	printf("Copyright 2013-2021 Broadcom Inc.  All rights reserved.\n\n");
	...
	while ((strlen(infname)>0) || !feof(list_fp) || multiFrameYuvFile)
	{
		...
		if (ip && (ip->chroma == YUV_420) && !dsc_codec.native_420)
		{
		...
		if (ip && (ip->chroma == YUV_422) && !(dsc_codec.simple_422 || dsc_codec.native_422 || dsc_codec.native_420))
		{
		...
		if (ip && (ip->color == RGB) && !dsc_codec.convert_rgb)
		{
		...
		else if (ip && ((ip->color == YUV_HD) || (ip->color == YUV_SD)) && dsc_codec.convert_rgb)      // YUV to RGB conversion
		{
		...
		if (ip && (ip->chroma == YUV_444) && (dsc_codec.simple_422 || dsc_codec.native_422 || dsc_codec.native_420))
		{
		...
		if (ip && (ip->chroma == YUV_422) && dsc_codec.native_420)
		{
		...
		if (ip && (dsc_codec.bits_per_component < ip->bits))
		{
		...
		if (ip && (ip->chroma == YUV_420) && dsc_codec.native_420) 
        {
			ip2 = pcreate_ext(FRAME, ip->color, YUV_444, ip->w/2, ip->h, ip->bits);
			pcopy_header(ip, ip2);
		...
		if(dsc_codec.native_420)
			op_dsc = (pic_t *)pcreate_ext(FRAME, (ip ? ip->color : YUV_HD), YUV_444, dsc_codec.pic_width/2, dsc_codec.pic_height, dsc_codec.bits_per_component);
		else if(dsc_codec.native_422)
			op_dsc = (pic_t *)pcreate_ext(FRAME, dsc_codec.convert_rgb ? RGB : (ip ? ip->color : YUV_HD), YUV_422, dsc_codec.pic_width, dsc_codec.pic_height, dsc_codec.bits_per_component);
		else
			op_dsc = (pic_t *)pcreate_ext(FRAME, dsc_codec.convert_rgb ? RGB : (ip ? ip->color : YUV_HD), YUV_444, dsc_codec.pic_width, dsc_codec.pic_height, dsc_codec.bits_per_component);
		if(ip)
			pcopy_header(ip, op_dsc);
		if (dsc_codec.convert_rgb)
		{
			...
			for (tpidx=0; tpidx<2; ++tpidx)
			{
				...
				temp_pic[tpidx] = (pic_t *)pcreate_ext(FRAME, YUV_HD, YUV_444, dsc_codec.pic_width, dsc_codec.pic_height, bitsPerComponent);
		...
		{
			ip2 = pcreate_ext(FRAME, op_dsc->color, YUV_422, op_dsc->w, op_dsc->h, op_dsc->bits);
        ...
        //SBR:OPTION 5
		if (dsc_codec.native_420) {
			ip2 = pcreate_ext(FRAME, op_dsc->color, YUV_420, op_dsc->w*2, op_dsc->h, op_dsc->bits);
			pcopy_header(op_dsc, ip2);
		...
		if (function!=1)  // Don't write if encode only
		{
			...
			if (hdrDpxFileOutput)
			{
				...
				memset(&hdrdpx_f, 0xff, sizeof(HDRDPXFILEFORMAT));
				if(transferFunction >= 0)
					op_dsc->transfer = transferFunction;
				if(colorimetry >= 0)
					op_dsc->colorimetry = colorimetry;
				if (colorDifferenceSiting >= 0)
					op_dsc->chroma_siting = colorDifferenceSiting;
		...
		if (ip)
		{
			...
			if (hdrDpxFileOutput)
			{
				...
				memset(&hdrdpx_f, 0xff, sizeof(HDRDPXFILEFORMAT));
				if (transferFunction >= 0)
					ref_pic->transfer = transferFunction;
				if (colorimetry >= 0)
					ref_pic->colorimetry = colorimetry;
				if (colorDifferenceSiting >= 0)
					ref_pic->chroma_siting = colorDifferenceSiting;
			...
			if(dsc_codec.native_422 || dsc_codec.native_420)
				fprintf(logfp, "\n%2.2f bits/(2-pixel pair), %d bits/component,", bitsPerPixel, bitsPerComponent);
			else
