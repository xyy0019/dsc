...
#define SINGLE_BS(x)  if(bswap) { size_t o; char t; o=offsetof(DPXFILEFORMAT,x); t=f_ptr[o]; f_ptr[o]=f_ptr[o+3]; f_ptr[o+3]=t; t=f_ptr[o+1]; f_ptr[o+1]=f_ptr[o+2]; f_ptr[o+2]=t; }
...
#define BYTE_SWAP(x)  x = (((x & 0xff) << 24) | ((x & 0xff00) << 8) | ((x & 0xff0000) >> 8) | (x >> 24))
	...
	for (b = 0; b < nbuffer; b++) // Loop over buffers
	{
		for (i = 0; i < hbuff[b]; ++i) // every row
		{
			...
			if ((pad_ends == 2 || (pad_ends == 1 && bpp == 8))
				&& (fifo.fullness > 0))
			{
				if(datum_order)
